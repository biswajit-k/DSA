...
void solve() {  
    ...
    llint rate = 2, max_packets = 10;

    // code

    llint curr_size = 0, dropped = 0;
    ...
    for(int i = 0; i < n; i++)
    {
        llint t = requests[i][0], p = requests[i][1];
        curr_size += p;
        if(curr_size > max_packets)
        {
            dropped += curr_size - max_packets;
            curr_size = max_packets;
        }

        if(i != n - 1)
        {
            llint tdiff = requests[i + 1][0] - requests[i][0];
            llint to_remove = min(curr_size, tdiff * rate);
            curr_size -= to_remove;
        }
